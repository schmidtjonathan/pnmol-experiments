...
def sparse_dense_blockdiag(A, B):
    ...
    sparse = pnmol.linops.BlockDiagonal.from_arrays(A, B)
    return sparse, dense
...
def test_matmul_blockdiag_blockdiag(sparse_dense_blockdiag, A, B):
    ...
    new = sparse @ sparse
    assert isinstance(new, pnmol.linops.BlockDiagonal)
...
def test_sum_block_diagonals(A, B):
    B1 = pnmol.linops.BlockDiagonal.from_arrays(A, B)
    B2 = pnmol.linops.BlockDiagonal.from_arrays(B, A)
    new = B1 + B2
...
def test_transpose_block_diagonals(A, B):
    BD = pnmol.linops.BlockDiagonal.from_arrays(A, B)
    new = BD.T
...
@pytest.fixture
def P0():
    return pnmol.linops.DerivativeSelection(derivative=0)


def test_is_operator(P0):
    assert isinstance(P0, pnmol.linops.DerivativeSelection)
...
def test_truncate_block_diagonal_array(d, n1, n2):
    ...
    dense_matrix = pnmol.linops.BlockDiagonal(array_stack).todense()
    dense_as_array_stack = pnmol.linops.truncate_block_diagonal(
        dense_matrix, num_blocks=d, block_shape=(n1, n2)
    )
    assert dense_as_array_stack.shape == (d, n1, n2)
    assert jnp.allclose(
        pnmol.linops.BlockDiagonal(dense_as_array_stack).todense(), dense_matrix
    )
