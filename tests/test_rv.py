...
class TestBatchedMultivariateNormal:
    ...
    def batched_multivariate_normal(batched_mean, batched_cov_sqrtm):
        return pnmol.rv.BatchedMultivariateNormal(
    ...
    def test_type(batched_multivariate_normal):
        assert isinstance(
            batched_multivariate_normal, pnmol.rv.BatchedMultivariateNormal
        )
    ...
    def test_jittable(batched_multivariate_normal):
        def fun(rv):
            m, sc = rv
            return pnmol.rv.BatchedMultivariateNormal(2 * m, 2 * sc)
...
class TestMatrixNormal:
    ...
    def matrix_normal(mean, cov_sqrtm):
        return pnmol.rv.MatrixNormal(
            mean=mean, cov_sqrtm_1=cov_sqrtm, cov_sqrtm_2=cov_sqrtm
        )

    @staticmethod
    def test_type(matrix_normal):
        assert isinstance(matrix_normal, pnmol.rv.MatrixNormal)
    ...
    def test_jittable(matrix_normal):
        def fun(rv):
            m, sc1, sc2 = rv
            return pnmol.rv.MatrixNormal(2 * m, 2 * sc1, 2 * sc2)
...
class TestLeftIsotropicMatrixNormal:
    ...
    def ek0_matrix_normal(mean, dimension, cov_sqrtm):
        return pnmol.rv.LeftIsotropicMatrixNormal(
            mean=mean, d=dimension, cov_sqrtm_2=cov_sqrtm
        )

    @staticmethod
    def test_type(ek0_matrix_normal):
        assert isinstance(ek0_matrix_normal, pnmol.rv.LeftIsotropicMatrixNormal)
    ...
    def test_jittable(ek0_matrix_normal):
        def fun(rv):
            m, d, sc2 = rv
            return pnmol.rv.LeftIsotropicMatrixNormal(2 * m, 2 * d, 2 * sc2)
