...
class BlockDiagonal(namedtuple("_BlockDiagonal", ["array_list"])):
    @property
    def num_blocks(self):
    ...
    def shape(self):
        return (
            sum((a.shape[0] for a in self.array_list)),
            sum((a.shape[1] for a in self.array_list)),
        )

    @property
    def T(self):
        transposed_array_list = [a.T for a in self.array_list]
    ...
    def __matmul__(self, other):
        if isinstance(other, jnp.ndarray):
            split_indices_colwise = jnp.cumsum(
                jnp.array([a.shape[1] for a in self.array_list])
            )[:-1]
            if other.ndim == 1:
                split_array = jnp.split(other, split_indices_colwise)
                individual_products = [
                    self.array_list[i] @ split_array[i] for i in range(self.num_blocks)
                ]
                ...
                out_block_rows = [
                    self.array_list[block_diag_index] @ r
    ...
    def __rmatmul__(self, other):
        if isinstance(other, jnp.ndarray):
            split_indices_rowwise = jnp.cumsum(
                jnp.array([a.shape[0] for a in self.array_list])
            )[:-1]
            if other.ndim == 1:
                split_array = jnp.split(other, split_indices_rowwise)
                individual_products = [
                    split_array[i] @ self.array_list[i] for i in range(self.num_blocks)
                ]
                return jnp.concatenate(individual_products)
                ...
                out_block_cols = [
                    c @ self.array_list[block_diag_index]
